...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(401)
      .json({ error: "Validation failed.(Message from server)" });
  } finally {
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!tag) {
      return res
        .status(404)
        .json({ error: "Tag not found.(Message from server)" });
    }
    ...
    return res
      .status(500)
      .json({
        error: "An error occurred while fetching the tag.(Message from server)",
      });
  } finally {
    await prisma.$disconnect();
  }
...
router.patch("/:id", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({
        error: "An error occurred while updating the tag.(Message from server)",
      });
  } finally {
    await prisma.$disconnect();
  }
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({
        error: "An error occurred while deleting the tag.(Message from server)",
      });
  } finally {
    await prisma.$disconnect();
  }
